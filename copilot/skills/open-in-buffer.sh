#!/bin/bash
#
# Opens one or more files as neovim buffers via the parent neovim's RPC socket.
# Use this when a file is referenced in conversation and should be visible to
# the user, regardless of whether it was modified.
#
# Usage: open-in-buffer.sh <file1> [file2] [file3] ...
#
...
if [[ $# -eq 0 ]]; then
  echo "usage: open-in-buffer.sh <file> [file...]" >&2
  exit 1
fi
